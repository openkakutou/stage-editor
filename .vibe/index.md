...
- No UI affordance is added for a capability that doesn't exist yet — no save/dirty-state indicator was added alongside this item's write-capable bridge, since no editing feature is wired up to it yet; a disabled or purely-visual stub is treated as worse than no affordance at all
...
- A write-path wrapper (`saveStage`) that re-parses a byte buffer internally (the original `.def` bytes, for its byte-exact-if-unchanged comparison) is tested for its own malformed-input error path, not assumed to inherit `load`'s — read the underlying Go source (`save.go`) to confirm this before writing the test, rather than guessing from the read-path's behavior
- Fixture-driven tests that exercise the real WASM module pin expected values obtained by running the actual module once against the fixture (not derived from the same mapping code under test), sharing a minimal `.def` fixture with `stage-viewer-web`'s own `src/wasm/testdata/` rather than depending on a sibling checkout at test time
