...
// (`OpenKakutouStage.load`/`OpenKakutouStage.save` — this app is write mode,
// unlike `stage-viewer-web`'s read-only bridge). Field names and shapes
// match the Go-side `json:"..."` tags exactly — see `stage`'s
// docs/data-model.md and docs/wasm.md. These are pure data types: no
// parsing/decoding logic lives here, mirroring `stage-viewer-web`'s own
// `src/wasm/types.ts` shape (itself mirroring `character-viewer-web`'s).
...
/**
 * Result of the typed bridge wrapper around `OpenKakutouStage.save`: exactly
 * one of `bytes`/`error` is ever meaningful. `bytes` is the serialized
 * `.def` file — byte-exact to the original when the edited stage describes
 * no real change, freshly generated text otherwise.
 */
