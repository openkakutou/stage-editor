...
// the browser. Ported verbatim (same success/error copy, byte-for-byte —
// no "editor-specific" wording, per UI/UX consultation) from
// `stage-viewer-web`'s own `src/input/stage-file-input-view.ts`; the only
// difference is what the caller does with a successful load (this repo
// stores it in `stage-document-store.ts` for future editor screens instead
// of just displaying it). No "ready to edit" affordance is added here —
// nothing is editable yet (item 003 hasn't landed), so any such signal
// would be a dead-end control.
//
// Backlog item 010 (i18n): the currently-displayed status/error text is
// kept as a small unformatted `StatusDescriptor`, not a pre-formatted
// string, so a live locale change (see .vibe/decisions/008) can re-format
// and redisplay it in the new language without re-running the load/parse
// that produced it — ported from `stage-viewer-web`'s own identical
// handling of this same, verbatim-shared component.
...
export function renderStageFileInput(
  ...
  // Live locale switching (backlog item 010): re-formats the currently
