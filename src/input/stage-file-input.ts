...
// folder listing — see backlog item 002. Ported verbatim from
// `stage-viewer-web`'s own `src/input/stage-file-input.ts` (only `loadStage`
// stays read-only here; `saveStage` isn't used by this item) — see that
// repo's `.vibe/decisions/001-sprite-sheet-resolved-by-basename-with-case-insensitive-fallback.md`
// for the resolution-rule rationale (exact match first, case-insensitive
// fallback second, real-corpus evidence from `character#050`), not
// re-derived here since nothing about it is specific to this repo.
