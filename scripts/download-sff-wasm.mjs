...
// openkakutou/sff for a pinned version tag, so this app can validate a BG
// element's sprite reference against a loaded stage's actual sprite sheet
// with no local Go toolchain. A separate script from download-wasm.mjs
// (which downloads `stage`'s own assets), not a parameterized shared one:
// each downloads a different producer repo's release, and — critically —
// lands in its own output subdirectory (`public/wasm/sff/`, not
// `public/wasm/`) so its own `wasm_exec.js` never collides with `stage`'s.
// A `wasm_exec.js` is only guaranteed compatible with the exact Go
// toolchain version that built the specific `.wasm` binary shipped next to
// it in the same release — see
...
// (ported verbatim, only the defaults differ).
